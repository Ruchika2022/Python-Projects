...
def showmon():
    ...
    new_gui.config(background = "white") 
    new_gui.title("CALENDER") 
    new_gui.geometry("200x100") 
    fetch_year = int(year1_field.get()) 
...
if __name__ == "__main__" : 
    ...
    year1_field = Entry(gui)
    ShowM = Button(gui, text = "Show Month", fg = "Black", bg = "Red", command = showmon) 
    ...
    month_field.grid(row=7,column=1)
    year1_field.grid(row=7,column=2)
    ShowM.grid(row=8,column=1)
